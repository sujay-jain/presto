        ...
        implements Filter
{
            ...
            throws IOException, ServletException
    {
        ...
        if (authorizers.isEmpty()) {
            log.debug("Skipping authorization for request %s %s %s", request.getMethod(), request.getRequestURI(), request.toString());
            nextFilter.doFilter(request, response);
            return;
        }

        log.debug("Applying authorization filter to request %s %s", request.getRequestURI(), request.toString());
        ...
        for (Authorizer authorizer : authorizers) {
            if (!authorizer.authorize(request)) {
                ...
                response.sendError(SC_FORBIDDEN, "Not Authorized to access this resource");
                return;
            }
        }
